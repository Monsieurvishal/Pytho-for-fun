...
#functions to get the mathematical operations done
...
def hcf(a,b):
    ...
    while H >= 1:
        if a%H == 0 and b%H == 0:
            return H
        H-=1
#extract the input from the text
    ...
    for word in text.split(' '):
        if word.upper() in operations.keys():
            try:
                ...
                list.delete(0,END)#delete previous result
                list.insert(END,r)#Insert new reults
            except:
                list.delete(0,END)
...
l2 = Label(win , text='I can add ,sub,mul,devide,get remainder,find HCF and LCM for you?' , padx=3)
l2.place(x=106,y=100)
